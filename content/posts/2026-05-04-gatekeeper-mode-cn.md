...
在解释设计之前，先说清楚实际流程长什么样——你作为使用者只需要做一件事：**写一份 brief（任务说明）**。剩下的由脚本驱动。

**一次 GateKeeper 运行的完整步骤：**

**① 你写 brief**  
一个 markdown 文件：要改什么、为什么改、允许动哪些文件（`allowed_files.txt`）。

**② Critic-Prep 先跑**（在 Builder 开始之前）  
Critic 读 brief，把验收标准写成 checklist——具体到函数签名、边界情况、哪些文件不能被动。**Builder 完全看不到这个 checklist。**

**③ Builder 在隔离的 git worktree 里工作**  
Builder 读 brief，写 patch，提交到独立分支。它不知道 checklist 是什么，只知道任务要求。

**④ Executor 跑确定性检查**  
编译 + 单元测试 + 评估器脚本。Exit code 非零直接触发 REJECT，不进入下一步。

> 测试从哪来？两种情况：  
> **改现有代码时**，跑项目里原有的测试套件，改坏了就抓到； 
> **新增功能时**，brief 要求 Builder 把代码和测试一起交，Critic checklist 里会明确列出"测试函数必须存在且通过"。  
> 评估器脚本是人预先写好的固定命令，不管 patch 改了什么，它只跑固定的几条指令——新写的测试只要在同一个文件里并被 `main()` 调用，就自动执行到。

**⑤ Critic-Review 对照 checklist 逐项核查**  
读 patch.diff + eval.log，每个 checklist item 要么有直接证据，要么标 FAILED。没有"看起来差不多"这种说法。

**⑥ Judge 写 final_decision.md**  
- `APPROVE`：所有 gate 通过，patch 合并
- `REJECT`：哪个 gate 失败、具体原因打包给 Builder，进入下一轮（最多 3 次）
- `ESCALATE`：超过重试上限，你收到完整的证据包，自己决定下一步

**你需要看的只有最后一个文件：`final_decision.md`。**
...
flowchart TD
    ...
    Orchestrator -->|Critic-Prep| C1[Critic<br/>读 brief + 目标文件<br/>写 review checklist]
    Orchestrator -->|Builder| B1[Builder<br/>读 brief + 目标文件]
    B1 --> B2[Builder<br/>写 patch]
    B2 --> B3{自检：patch 是否符合 brief?}
    B3 -->|不符合| B2
    B3 -->|符合 — 提交| Exec
    ...
    T3 -->|失败| Rej
    T3 -->|通过| T4[跑语义不变量检查]
    T4 --> T5{输出与 baseline 一致?}
    T5 -->|不一致| Rej
    T5 -->|一致| C2

    C1 -.共享 checklist.-> C2
    C2[Critic-Review<br/>对照 checklist 逐项核查<br/>patch + 执行日志]
    C2 --> C3{默认拒绝：<br/>有 checklist item<br/>缺少直接证据?}
    C3 -->|有 — 必须拒绝| Rej
    C3 -->|没有 — 每项都有证据| Pkg

    Rej[REJECT<br/>日志 + Critic 笔记 + 失败 gate]
    Rej -->|第 N+1 次尝试<br/>最多 3 次| B2
    Rej -.->|超过重试上限| Esc([ESCALATE 给人])

    Pkg[决策包<br/>diff + 执行命令 + 裁决]
    Pkg --> Human([人只读最终裁决])
    Esc --> Human
...
| 蓝色 | Builder | 尽力实现任务 |
| 红色 | Critic | 尽力找漏洞 |
| 黄色 | Executor | 用机器输出替代 LLM 判断 |

**最关键的设计：Critic 默认拒绝。** 一个默认"看起来没问题"的 Critic 没有任何价值，它只是个附和者。系统必须强制把举证责任压在 patch 上，而不是压在 rejection 上。

---

## 3. 为什么不能让一个 agent 同时做这三件事

这个问题值得单独回答，因为"为什么不直接让一个 Claude 又写又审"这个疑问几乎人人都有。

| 职责 | 如果由同一个 agent 完成 |
|------|----------------------|
| Builder | 它写出一个追求"看起来合理"的 patch |
| Critic | 它在为自己的实现做辩护 |
| Executor | 它报告的是"agent 说发生了什么"，不是"机器实际测到了什么" |

三种失败模式合并成一种：整个循环变成自我确认叙事。

**Executor 必须是确定性的机器输出**（编译成功/失败、测试通过/失败），不能是 LLM 的判断。这是 GateKeeper 和"让 AI 自己看看对不对"最本质的区别。

---

## 4. Critic-Prep：最关键的升级
...
这已经比纯手动好——执行结果是确定性的，Critic 的评审有日志可以引用。但问题在于：**Critic 的判断标准在看到 patch 之后才形成。** 它很难真正独立于 Builder 的实现方式。
...
Critic-Prep 发生在 Builder 开始工作之前。它读 brief 和允许修改的文件范围，把验收 checklist 写出来——精确到函数签名、边界情况的期望返回值、哪些文件不能被动。
...
为什么不用 AutoGen 或 LangGraph？不是因为它们不好，而是在还没有确定性证据证明 workflow 的语义正确之前，引入框架只会增加调试难度。先用可见的文件和日志把语义搞清楚，再考虑迁移。
...
四个门禁是串行的。通过前一个才能进入下一个。失败越早发现，成本越低。

<!--
╔══════════════════════════════════════════════════════════════════╗
║  🖼  ILLUSTRATION #2  ——  四个门禁串行流程图（文生图）          ║
╠══════════════════════════════════════════════════════════════════╣
║                                                                  ║
...
Attempt 1 直接通过。当证据标准足够清晰，一次通过不是运气，是必然。
...
重试机制的关键设计：REJECT 不只是返回"失败"，它把完整的证据携带给下一轮 Builder。Builder 下一轮看到的是：哪个 gate 失败了、失败原因是什么、哪些 checklist item 没有找到证据。这比单纯的"重试"有信息量得多——Builder 不是盲目重试，而是带着诊断结果重试。
...
ESCALATE 不是"流程出错了"，而是系统在说：这个任务超出了当前可以自动处理的范围，需要人来看。`final_decision.md` 附带 cleanup 命令和所有中间 artifact 的路径，方便直接定位问题。人审查证据包，不需要重新读对话历史。
...
**这是 Critic-Prep 的核心价值所在。** 它在看任何代码之前就把"只捕获特定异常"写进了 checklist。没有这个预写的 checklist，这种语义问题在评审里几乎一定会被忽略——"4/4 测试通过，代码看起来也合理"，批准的门槛太低了。

这个场景还验证了一件更重要的事：**如果你只看 Executor gate，你会以为 patch 已经通过了。语义正确性需要 Critic gate，两个门禁缺一不可。**
...
checklist 覆盖了文件范围、测试函数名、测试逻辑的三个具体验证点、构建通过状态、生产文件不能被改动。每一项都是可以在 patch.diff 或 eval.log 里找到直接证据的，没有一项是"大体满足就算"的主观判断。
...
Builder 在 Attempt 1 里验证了剩余量，但没有显式断言成交量等于期望值——`executed_quantity + remaining_volume == total_submitted_quantity` 这个等式只验证了右边，没有验证左边。Critic-Prep 的 checklist 里有这一项，遗漏被准确定位。Attempt 2 补上了这个断言，通过。

这类遗漏在普通 code review 里极容易漏掉：实现看起来完整、测试也通过了，只有对照预先写下的验收标准逐项检查，才能发现这个漏洞。
...
**任务**：修复 strategy 层的 ownership 验证和 fill accounting。比 invariant 测试更复杂，涉及多个文件。
...
这次 ESCALATE 是有价值的结果，不是流程失败。**GateKeeper 不只是 patch 过滤器：它是调试质量系统本身的工具。** 如果没有 Critic 在证据层面较真，`-DNDEBUG` 这个问题可能会以"行为奇怪但测试通过"的方式潜伏很长时间。三次 ESCALATE 暴露了评估器自身的质量问题——这件事本身比任何一次 APPROVE 都有价值。

<!--
╔══════════════════════════════════════════════════════════════════╗
║  🖼  ILLUSTRATION #3  ——  -DNDEBUG 漏洞示意图（文生图）        ║
╠══════════════════════════════════════════════════════════════════╣
║                                                                  ║
...
解决方案是两阶段 API：`prepare_order()` 先同步返回 `order_id`，strategy 记录之后再调用 `submit_prepared_order()` 触发撮合。Critic-Prep 写下的 checklist 要求"有证据证明 on_trade 能正确识别自己的 fill"，这条要求强迫实现者必须面对时序问题，没有绕开的空间。
...
**它不能替代领域测试。** GateKeeper 保证 checklist 里的项有证据，但 checklist 的质量取决于 Critic-Prep 的输入。如果 brief 写得不完整，checklist 就不会覆盖所有需求。评估器测了什么，就只能保证什么。

**它不能证明 agent 永远正确。** 通过的 patch 是"在当前 checklist 和测试覆盖下被接受的 patch"，不是"永远正确的 patch"。这两件事不一样，不应该混淆。

**Attacker 还没接入。** 完整的设计里，Attacker 角色应该在 Critic-Review 之前主动用边界输入攻击 patch，把攻击结果作为额外证据送给 Critic。当前的质量门禁只有防御性的，没有主动攻击性的。

最后一条，也是最重要的一条：

> **一个 Executor 门禁只有它运行的命令那么强。如果测试依赖 `assert()`，在 `-DNDEBUG` 下运行就把质量门禁变成了冒烟测试。**

这不是 GateKeeper 的问题，是使用者的责任。GateKeeper 的价值在于让这种弱点变得可见——但修复需要使用者动手。

---

## 10. 为什么这对 Agent 优化很重要

如果你想让 agent 帮你做性能优化——减少内存分配、优化热路径、调整数据结构布局——你需要先确认一件事：**你有没有能力证明优化之后的代码行为和优化之前一致？**

性能优化的本质是在不改变语义的前提下改变实现。如果正确性门禁还是模糊的，优化之后的测试通过只能说明"能跑通"，不能说明"语义没变"。你很可能得到一个跑得更快但行为已经悄悄变了的版本，还很难追溯是哪一步引入了问题。
...
从一个你已经有测试的小任务开始：写一个 brief，让 Critic-Prep 生成 checklist，再让 Builder 工作——**在看 Builder 的实现之前，先读一遍 checklist**。这个顺序本身就能让你感受到"证据标准先于 patch"和"有了 patch 再回头评审"之间的区别。
